...
namespace MVCAdminApplication.Controllers
{
    public class OrderController : Controller
    {
        ...
        public FileContentResult CreateInvoice(Guid Id)
        {
            ...
            document.Content.Replace("{{UserName}}", data.Owner.Email);
            StringBuilder sb = new StringBuilder();
            var total = 0.0;
            foreach (var item in data.ProductInOrders)
            {
                sb.Append("Ticket for movie " + item.OrderedProduct.Movie.MovieName+ " with quantity " + item.Quantity + " with price " + item.OrderedProduct.Price + "$\n");
        ...
        public FileContentResult ExportOrders()
        {
            ...
            using (var workBook = new XLWorkbook())
            {
                ...
                for (int i = 0; i < data.Count(); i++)
                {
                    ...
                    {
                        worksheet.Cell(1, j + 3).Value = "Movie - " + (j + 1);
